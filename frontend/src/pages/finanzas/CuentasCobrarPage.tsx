...
import { useCuentasCobrar, useCreateCuentaCobrar, usePagarCuentaCobrar } from '@/services/finanzas.service'
import { LoadingSpinner } from '@/components/shared/LoadingSpinner'
import type { CuentaPorCobrar, CuentaCobrarCreate, PagoCreate } from '@/types/finanzas'
...
export default function CuentasCobrarPage() {
  ...
  const { data, isLoading, refetch } = useCuentasCobrar({ skip, limit, estado: estado || undefined })
  const createMutation = useCreateCuentaCobrar()
  const [showForm, setShowForm] = useState(false)
  const [form, setForm] = useState<CuentaCobrarCreate>({ cliente_id: '', concepto: '', monto_total: 0, fecha_emision: new Date().toISOString().split('T')[0] })
  ...
  const handleCreate = async () => {
    ...
    setForm({ cliente_id: '', concepto: '', monto_total: 0, fecha_emision: new Date().toISOString().split('T')[0] })
    setShowForm(false)
    refetch()
  }
  ...
  const handlePago = async (cuentaId: string) => {
    ...
    await api.post(`/api/v1/finanzas/cobrar/${cuentaId}/pagos`, pagoForm)
      ...
      {showForm && (
        <div className="card p-4 mb-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input type="text" placeholder="Cliente ID" value={form.cliente_id} onChange={(e) => setForm((p) => ({ ...p, cliente_id: e.target.value }))} className="input-field" />
              ...
              <table className="w-full text-sm">
                <thead className="bg-secondary-50">
                  <tr>
                    ...
                    <th className="text-right p-3 font-semibold text-secondary-600">Pagado</th>
                ...
                <tbody>
                  {items.map((c: CuentaPorCobrar) => (
                    <>
                      <tr key={c.id} className="border-t border-secondary-100">
                        ...
                        <td className="p-3 text-right text-success">${Number(c.monto_pagado).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                        <td className="p-3 text-right font-semibold text-secondary-800">${Number(c.saldo).toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                        <td className="p-3">{c.estado !== 'pagada' && <button onClick={() => { setPagandoCuenta(c.id); setPagoForm({ monto: c.saldo, fecha: new Date().toISOString().split('T')[0] }) }} className="text-xs btn-outline py-0.5 px-2">Registrar pago</button>}</td>
                      </tr>
                      {pagandoCuenta === c.id && (
                  ...
                  {items.length === 0 && <tr><td colSpan={7} className="p-6 text-center text-secondary-400">No hay cuentas por cobrar</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
